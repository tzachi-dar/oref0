...
function reduce_boluses (treatments, iobFileName) {
  ...
  function bolus (ev, remaining) {
    ...
    if (remaining && remaining.length > 0) {
      ...
      if (has_insulin) {
        var fs = require('fs');
        if (fs.existsSync(iobFileName)) {
            var iob = JSON.parse(fs.readFileSync(iobFileName));
...
exports = module.exports = reduce_boluses;
